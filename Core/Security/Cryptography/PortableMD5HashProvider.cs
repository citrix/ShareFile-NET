...
{
    internal class PortableMD5HashProvider : IMD5HashProvider
    {
        static PortableMD5HashProvider()
        {
            MD5HashProviderFactory.RegisterHashProvider(() => new PortableMD5HashProvider());
        }

        private MD5Digest _md5Digest;
        ...
        {
            _md5Digest.BlockUpdate(buffer, offset, size);
        }

        public void Finalize(byte[] buffer, int offset, int size)
        {
            _md5Digest.BlockUpdate(buffer, offset, size);

            _md5Digest.DoFinal(_computedHash, 0);
        }

        public string GetComputedHashAsString()
        {
            return BytesToHex(_computedHash);
        }
        ...
        {
            _md5Digest.Reset();
            Finalize(buffer, 0, buffer.Length);
